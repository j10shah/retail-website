...
const { getUserByUsername, createUser, createBusinessUser, getUserByEmail, getBusinessUserByEmail } = require('./db'); // Import the db functions
...
async function handleBusinessLogin(req, res) {
    ...
    req.on('end', async () => {
        ...
        try {
            // Fetch user by email instead of username
            const user = await getBusinessUserByEmail(email);  // Updated function to fetch by email
...
async function registerBusinessUser(req, res) {
    ...
    req.on('end', async () => {
        try {
            ...
            const passwordHash = await bcrypt.hash(password, 10);
            const userId = await createBusinessUser(email, passwordHash); // Use createUser to insert into DB
            res.writeHead(201, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ message: 'User registered successfully', userId }));
        } catch (error) {
            console.error('Error during registration:', error.message);
